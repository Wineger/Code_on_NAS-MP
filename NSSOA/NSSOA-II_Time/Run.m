...
Cxy = 1.07;   % Motion trajectory parameter
...
weight = [0.5, 0.5];  % Objective weights
...
limit = inf;  % Limit the number of adjustment layers

% Optimization result matrix
result1 = zeros(2, day);   % Without optimization index
...
disp(['Sensor initial optimization deployment completed: F1 member count = ' num2str(numel(F1)), ' Initial coverage ratio = ', num2str(Init_reslt(1, 1)), ' Initial connectivity ratio = ', num2str(Init_reslt(2, 1))]);
...
for i = 1:day
    ...
    % Without optimization iteration
    ...
    % Population optimization iteration
...
% Save results
save('result.mat', 'result1', 'result2');
load result.mat
...
totalTime = toc;
disp(['Total time: ', num2str(totalTime / (60 * 60)), ' hours']);
