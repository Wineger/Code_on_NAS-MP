...
function ImportData(filename)
    ...
    for i = 1:length(info.Variables)
        ...
        var_size = [info.Variables(i).Dimensions.Length]; % Get variable dimensions size
    ...
    % Update longitude and latitude data directly in the structure
    ...
    % Assume this is the target interpolation grid
    ...
    % Update longitude and latitude data in the structure to the interpolation grid longitude and latitude
    ...
    % Update uo and vo data in the structure to the interpolated result
    Data.uo = uo_interp;
    Data.vo = vo_interp;
    ...
    % Convert the generated grid to column vectors so that all combinations of each point are in one row
    Lon = Lon(:);
    Lat = Lat(:);
    Dep = Dep(:);
    
    % Combine these three column vectors into one matrix
    data = [Lon, Lat, Dep];
    ...
    for idx = 1:size(data, 1)
        % Extract longitude, latitude, and depth for the current position
        current_lon = data(idx, 1);
        current_lat = data(idx, 2);
        current_depth = data(idx, 3);
        
        % Find matching indices in longitude, latitude, depth
        lon_index = find(abs(longitude - current_lon) < 1e-5, 1); % Use precision threshold to avoid floating point errors
    ...
    data(:,1) = data(:,1) * ((9*1000)/(longitude_range(2)-longitude_range(1))) * (600/(9*1000));  % Scale area
    data(:,2) = data(:,2) * ((9*1000)/(latitude_range(2)-latitude_range(1))) * (600/(9*1000));
    data(:,3) = data(:,3);
    
    U = U * (600/(9*1000));  % Scale speed
    V = V * (600/(9*1000));
